...
// ==============================================
var (
	VERSION    = "2.5.1" // Incremented version for the fix
...
func main() {
	...
	if config.Verbose {
		fmt.Printf("%s[INFO]%s Loaded %d patterns. Initial source: %s\n",
...
func (s *Scanner) loadPatterns() error {
	...
	for _, sourceName := range patternFileSources {
		...
		if !opened {
			// Log warning and continue if in verbose, otherwise just skip this problematic file
			if s.Config.Verbose {
				fmt.Printf("%s[WARN]%s Cannot open pattern file '%s' in any location. Skipping.\n", ColorYellow, ColorReset, sourceName)
			}
			// Optionally, could return an error here if strict loading is preferred:
			// return fmt.Errorf("cannot open pattern file '%s' in any location", sourceName)
			continue
		}
		defer file.Close() //
		...
		for fileScanner.Scan() {
			...
			if err != nil {
				if s.Config.Verbose {
					fmt.Printf("%s[WARN]%s Invalid regex at line %d in %s: %s. Skipping pattern.\n",
						ColorYellow, ColorReset, lineNum, usedPath, err) //
				}
		...
		if err := fileScanner.Err(); err != nil {
			// Log warning and continue if in verbose, otherwise just skip this problematic file
			if s.Config.Verbose {
				fmt.Printf("%s[WARN]%s Error reading file %s: %v. Patterns loaded so far from this file might be incomplete.\n",
					ColorYellow, ColorReset, usedPath, err)
			}
			// Optionally, could return an error here if strict loading is preferred:
			// return fmt.Errorf("error reading file %s: %v", usedPath, err)
			continue
		}
	...
	if s.Config.Verbose && filesSuccessfullyProcessed < len(patternFileSources) {
		fmt.Printf("%s[INFO]%s Successfully processed patterns from %d out of %d specified source(s).\n",
...
func (s *Scanner) showFinalStats() {
	...
	// FIX: Avoid division by zero for speed calculation
	speed := 0.0
	...
	fmt.Printf("%s║%s  🚀 Speed:          %s%-10.1f URLs/sec%s           %s║%s\n",
		ColorPurple, ColorReset, ColorCyan, speed, ColorReset, ColorPurple, ColorReset) // // Uses the fixed speed
	fmt.Printf("%s╠══════════════════════════════════════════════════════════╣%s\n", ColorPurple, ColorReset) //
...
// ==============================================
// GRACEFUL PANIC HANDLING (RECOMMENDATION)
// ==============================================
// It's generally better to handle errors explicitly.
// The `init()` based recover was very limited in scope.
// If a panic occurs in a goroutine, it will terminate that goroutine.
// For a more robust application-wide panic handler, you might consider
// a deferred function in main or at the start of each goroutine if necessary,
// but this is often a sign of underlying issues that should be fixed.
// For this tool, explicit error handling is preferred.
// The previous init() based recover is removed as it wasn't effective for runtime panics in main/goroutines.
// A panic during `regexp.Compile` is handled by skipping the pattern.
// A panic during I/O operations should ideally be caught as errors by Go's standard library.
